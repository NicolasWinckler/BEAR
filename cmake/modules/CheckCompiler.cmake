...
If(FAIRSOFT_CONFIG)
  ...
  Set(BEAR_C_COMPILER ${CMAKE_C_COMPILER})
  Set(BEAR_CXX_COMPILER ${CMAKE_CXX_COMPILER})
  Set(BEAR_Fortran_COMPILER ${CMAKE_Fortran_COMPILER})
  String(STRIP ${BEAR_C_COMPILER} BEAR_C_COMPILER)
  String(STRIP ${BEAR_CXX_COMPILER} BEAR_CXX_COMPILER)
  If(BEAR_Fortran_COMPILER)
    String(STRIP ${BEAR_Fortran_COMPILER} BEAR_Fortran_COMPILER)
    Get_Filename_Component(BEAR_Fortran_COMPILER ${BEAR_Fortran_COMPILER} REALPATH)
  EndIf()

  Get_Filename_Component(BEAR_C_COMPILER ${BEAR_C_COMPILER} REALPATH)
  Get_Filename_Component(BEAR_CXX_COMPILER ${BEAR_CXX_COMPILER} REALPATH)

  If(NOT (${FAIRSOFT_C_COMPILER} STREQUAL ${BEAR_C_COMPILER}) OR NOT (${FAIRSOFT_CXX_COMPILER} STREQUAL ${BEAR_CXX_COMPILER}))
    Message(STATUS "C compiler used for FairSoft installation:  ${FAIRSOFT_C_COMPILER}")
    Message(STATUS "C compiler used now:  ${BEAR_C_COMPILER}")
    Message(STATUS "CXX compiler used for FairSoft installation:  ${FAIRSOFT_CXX_COMPILER}")
    Message(STATUS "CXX compiler used now:  ${BEAR_CXX_COMPILER}")
  ...
  If(BEAR_Fortran_COMPILER) 
    If(NOT (${FAIRSOFT_Fortran_COMPILER} STREQUAL ${BEAR_Fortran_COMPILER}))
      String(STRIP ${FAIRSOFT_Fortran_COMPILER} FAIRSOFT_Fortran_COMPILER)
      Message(STATUS "Fortran compiler used for FairSoft installation:  ${FAIRSOFT_Fortran_COMPILER}")
      Message(STATUS "Fortran compiler used now:  ${BEAR_Fortran_COMPILER}")
      Message(STATUS "The compiler during the compilation of FairSoft is different from the current one.")
      If(USE_DIFFERENT_COMPILER)
        Message(STATUS "The error was silenced by the usage of -DUSE_DIFFERENT_COMPILER=TRUE")
      Else()
